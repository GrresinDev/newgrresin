<script lang="ts">
	...
	let nav_list: NavItem[] = [
		{ text: m.muddy_lofty_yak_dart(), href: localizeHref('/about') },
		{
			href: localizeHref('/#contact'),
			text: m.early_clean_cod_devour()
		},
		{ text: m.deft_cuddly_jaguar_jump(), href: localizeHref('/blog') },
		{ text: m.dry_lazy_lamb_dine(), href: localizeHref('/order') },
		{ text: m.loud_elegant_hyena_adapt(), href: localizeHref('/projects') },
		{ text: m.stock_careful_jackal_urge(), href: localizeHref('/careers') }
	];
</script>
...
{#snippet uk()}
	<svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 32 32"
		...
		></path><path
			...
			fill="#fff"
		></path><rect x="13" y="4" width="6" height="24" fill="#fff"></rect><rect
			x="1"
			y="13"
			width="30"
			height="6"
			fill="#fff"
		></rect><rect x="14" y="4" width="4" height="24" fill="#b92932"></rect><rect
			x="14"
			y="1"
			width="4"
			height="30"
...
{#snippet ksa()}
	<svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 32 32"
		><rect x="1" y="4" width="30" height="24" rx="4" ry="4" fill="#215230"></rect><path
			d="M27,4H5c-2.209,0-4,1.791-4,4V24c0,2.209,1.791,4,4,4H27c2.209,0,4-1.791,4-4V8c0-2.209-1.791-4-4-4Zm3,20c0,1.654-1.346,3-3,3H5c-1.654,0-3-1.346-3-3V8c0-1.654,1.346-3,3-3H27c1.654,0,3,1.346,3,3V24Z"
			opacity=".15"
		></path><path
			d="M27,5H5c-1.657,0-3,1.343-3,3v1c0-1.657,1.343-3,3-3H27c1.657,0,3,1.343,3,3v-1c0-1.657-1.343-3-3-3Z"
...
{/snippet}
<!--Haeder-->
<header class="text-accent w-full border-b bg-black" id="navigation">
	<div class="felx-row flex h-16 items-center justify-between px-4">
		...
		>
			<div class="rounded-lg p-2 2xl:hidden">
				<enhanced:img src={logo} alt="Graffiti Resin Logo" class="h-10 w-10 object-cover" />
			</div>
			<div class="hidden 2xl:block">
				<enhanced:img src={logo} alt="Graffiti Resin Logo" class="h-14 w-14 object-cover" />
			</div>
		</a>
		...
		<div class="mx-4 hidden w-1/3 flex-1 items-start justify-start md:flex">
			<nav class="flex gap-6" aria-label="Main navigation">
				{#each nav_list as navItem}
					<Button href={navItem.href} variant="ghost">{navItem.text}</Button>
				{/each}
			</nav>
		</div>
		...
		<div class="flex w-1/3 items-center justify-end gap-4">
			<Button
				class="hidden bg-white text-[#a71580] capitalize hover:bg-[#a71580] hover:text-white md:flex"
				onclick={() => {
					setLocale('en');
				}}
			>
...
<Sheet.Root bind:open>
	<Sheet.Content {side} class="8 w-64 bg-white text-black">
		<div class="mx-4 mt-8">
			<div class="mb-6 flex items-center justify-start gap-x-3">
				...
				<span class="text-md font-semibold" id="mobile-menu-title">Menu</span>
			</div>
			<nav class="flex flex-col space-y-1" aria-label="Mobile navigation">
				...
				<Button
					size="sm"
					class="justify-start bg-white text-[#a71580] capitalize hover:bg-[#a71580] hover:text-white"
					onclick={() => {
						setLocale('en');
